...
from packages.eightballer.skills.reset_pause_abci import PUBLIC_ID
from packages.valory.skills.abstract_round_abci.base import (
    AbciAppDB,
    BaseSynchronizedData as ResetSynchronizedSata,
)
from packages.eightballer.skills.reset_pause_abci.rounds import Event as ResetEvent, FinishedResetAndPauseRound
from packages.eightballer.skills.reset_pause_abci.behaviours import ResetAndPauseBehaviour
...
class TestResetAndPauseBehaviour(ResetPauseAbciFSMBehaviourBaseCase):
    ...
    def test_reset_behaviour(
        self,
        tendermint_reset_status: bool | None,
    ) -> None:
        """Test reset behaviour."""
        dummy_participants = [list(range(4))]
        ...
        assert self.behaviour.current_behaviour.behaviour_id == self.behaviour_class.auto_behaviour_id()
        ...
        with (
            ...
            mock.patch.object(
                ...
                side_effect=dummy_reset_tendermint_with_wait_wrapper(tendermint_reset_status),
            ) as mock_reset_tendermint_with_wait,
            mock.patch.object(
                self.behaviour.current_behaviour,
            ...
            if tendermint_reset_status is not None and not tendermint_reset_status:
                mock_reset_tendermint_with_wait.side_effect = dummy_reset_tendermint_with_wait_wrapper(True)
                self.behaviour.act_wrapper()
                # make sure that the behaviour does not send any txs to other agents when Tendermint reset fails
                assert isinstance(self.behaviour.current_behaviour.send_a2a_transaction, MagicMock)
